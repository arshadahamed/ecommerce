                                    ...
                                    <tbody>
                                        @foreach ($categories as $category)
                                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                                ...
                                                <td class="px-6 py-4">
                                                    <!-- Edit Button -->
                                                    <a href="{{ route('show.cat', $category->id) }}" class="btn btn-sm btn-success rounded">Edit</a>

                                                    <!-- Delete Button -->
                                                    <form action="{{ route('delete.cat', $category->id) }}" method="POST" style="display:inline;">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn btn-sm btn-danger rounded" onclick="return confirm('Are you sure you want to delete this category?');">Delete</button>
                                                    </form>
                                                </td>
                                            </tr>
                                        @endforeach
