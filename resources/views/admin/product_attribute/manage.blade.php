                                    ...
                                    <tbody>
                                        @foreach ($attributes as $attribute)
                                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                                ...
                                                <td class="px-6 py-4">
                                                    <!-- Edit Button -->
                                                    <a href="{{ route('show.attribute', $attribute->id) }}" class="btn btn-sm btn-success rounded">Edit</a>

                                                    <!-- Delete Button -->
                                                    <form action="{{ route('delete.attribute', $attribute->id) }}" method="POST" style="display:inline;">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn btn-sm btn-danger rounded" onclick="return confirm('Are you sure you want to delete this Attribute?');">Delete</button>
                                                    </form>
                                                </td>
                                            </tr>
                                        @endforeach
